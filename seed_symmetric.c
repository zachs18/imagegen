	...
	if (seedcount > 0) {
		...
		while (realseedcount < seedcount) {
			...
			for (int i = 0; i < floodplanecount; ++i) {
				...
				realseedcount += seed_floodplane_symmetric(&floodplanes[i], data, used_, try);
			}
			for (int i = 0; i < floodplanecount; ++i) {
				int try = (seedcount - realseedcount) / (floodplanecount - i); // never x/0
				realseedcount += seed_floodplane_symmetric(&floodplanes[floodplanecount-i-1], data, used_, try);
			}
		}
	...
	for (int x = 0; x < dimx; ++x) {
		for (int y = 0; y < dimy; ++y) {
			if (!filled[y][x] && !blocked[y][x]) {
				...
				floodplanes[floodplanecount-1].count = flood_iterative_symmetric(dimx, dimy, x, y, floodplanes[floodplanecount-1].pixels, (bool*) filled, blocked_);
...
static struct pixel *flood_recursive_symmetric(int dimx, int dimy, int cx, int cy, struct pixel *pixels, bool *filled_, bool *blocked_) {
	...
	for (int i = 0; i < floodoffsetcount; ++i) {
		...
		if (x >= 0 && x < dimx && y >= 0 && y < dimy && !filled[y][x] && !blocked[y][x])
			pixels = flood_recursive_symmetric(dimx, dimy, x, y, pixels, filled_, blocked_);
