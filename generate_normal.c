...
static double outer_fitness_normal_best(int dimx, int dimy, __m128 *values_, bool *used_, struct pixel pixel, const __m128 color);
static double outer_fitness_normal_average(int dimx, int dimy, __m128 *values_, bool *used_, struct pixel pixel, const __m128 color);
static double outer_fitness_normal_worst(int dimx, int dimy, __m128 *values_, bool *used_, struct pixel pixel, const __m128 color);

static double (*outer_fitness_normal)(int dimx, int dimy, __m128 *values_, bool *used_, struct pixel pixel, const __m128 color) = outer_fitness_normal_average;
...
static void *generate_outer_worker_normal(void *gdata_) {
	...
	while (*pixels < size) { // datalock doesn't need to be locked here for pixels?
		...
		for (int i = start; i < end; ++i) {
			for (int c = 0; c < colorcount; ++c) {
				double fitness = outer_fitness_normal(dimx, dimy, (__m128*) values, (bool*) used, edgelist->edges[i], colors[c]);
				if (fitness < myfitnesses[c]) {
					myfitnesses[c] = fitness;
					mybests[c] = i;
				}
...
double outer_fitness_normal_best(int dimx, int dimy, __m128 *values_, bool *used_, struct pixel pixel, const __m128 color) {
	bool (*used)[dimx] = (bool(*)[dimx]) used_;
	double ret = DBL_MAX;
	for (int i = 0; i < offsetcount; ++i) {
...
double outer_fitness_normal_average(int dimx, int dimy, __m128 *values_, bool *used_, struct pixel pixel, const __m128 color) {
	bool (*used)[dimx] = (bool(*)[dimx]) used_;
	double ret = 0;
	int found = 0;
	for (int i = 0; i < offsetcount; ++i) {
...
double outer_fitness_normal_worst(int dimx, int dimy, __m128 *values_, bool *used_, struct pixel pixel, const __m128 color) {
	bool (*used)[dimx] = (bool(*)[dimx]) used_;
	double ret = 0;
	for (int i = 0; i < offsetcount; ++i) {
		int tx = pixel.x + offsets[i].dx, ty = pixel.y + offsets[i].dy;
